...
# we remove the first 2 lines:
# We did this because the first lines contains the SUM of the metrics. It has data about the US as total
# We removed the second line because the Alabama is a state and the level of detail is on county level.
county_facts <- county_facts[-c(1:2), ]
...
plot(silhouette(cutree(hc1, k = 5), dist(x)), col=2:6,main ='ward',border = NA)
plot(silhouette(cutree(hc1, k = 4), dist(x)), col=2:5,main ='ward',border = NA)
...
plot(silhouette(cutree(manh1, k = 5), dist(x, method = 'manhattan')), col=2:6,main ='ward',border = NA)

plot(manh1)
rect.hclust(manh1, k=2, border="red")
...
# Mahalonibis distance could not be calculated. I tried but i cound not find the solution ! 
...
trol <- hclust(dist(x, method = 'manhattan'), method="ward.D")
plot(trol)

plot(silhouette(cutree(hc1, k = 3), dist(x, method = 'manhattan')), border = NA)
plot(silhouette(cutree(trol, k = 3), dist(x, method = 'manhattan')), border = NA)
